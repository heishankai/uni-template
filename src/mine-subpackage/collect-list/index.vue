<template>
  <view class="container">
    ...
    <view class="navbar" :style="{ paddingTop: safeAreaInsets!.top + 'px' }">
      ...
      <view class="tips">我的收藏</view>
      <view class="search">
        <view class="search-input">
          <uni-icons type="search" color="#00cec9" size="24" />
          <input placeholder="请输入名称查询" @input="handleInputChange" />
        </view>
      </view>
    </view>
...
// services
import { collectListService } from './service'
// components
...
// 撰稿人列表数据
const allWriterListdata = ref<any[]>([]) // 所有数据的备份
const writerListdata = ref<any[]>([]) // 当前展示的数据
...
const handleInputChange = debounce(async (e: any): Promise<any> => {
  ...
  if (!keyword) {
    writerListdata.value = allWriterListdata.value
    return
  }

  writerListdata.value = allWriterListdata.value.filter((item) =>
    item.nickname?.toLowerCase().includes(keyword),
  )
}, 300)

// 处理子组件传递的更新数据
const handleUpdateWriterListdata = (): any => getCollectList()
...
// 返回上一页
const handleBack = (): any => uni.navigateBack()
